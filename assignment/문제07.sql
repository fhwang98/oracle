...
WHERE age > (SELECT avg(age)
			FROM tbladdressbook
				WHERE gender = 'm' AND hometown = '서울' AND job IS NOT NULL);
...
(SELECT job, max(count(*)) FROM tbladdressbook GROUP BY job)
...
SELECT
	substr(email, instr(address, '@') + 1)
FROM tbladdressbook;
	GROUP BY substr(email, instr(address, '@') + 1);
